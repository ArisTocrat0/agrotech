...
def main():
    ...
    report={'operation':'class-aware NMS only','boxes':1500,'classes':5,'seed':42,
            ...
            'native_seconds':timings['auto'],'speedup':timings['python']/timings['auto'],
            'platform':platform.platform(),'scope':'Does not measure DINOv2 or full-frame FPS.'}
    output=Path('artifacts/benchmark_nms.json');output.write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
